...
describe('api', () => {
	describe('getDrivers', () => {
		...
		it('should return an empty array when fetch throws an error', async () => {
			let result;
			try {
				global.fetch = jest.fn().mockImplementation(() => Promise.reject('Error fetching'));
				result = await getDrivers('2019');
			} catch(e) {
				expect(console.error).toHaveBeenCalledWith('Error while fetching getDrivers: Error fetching');
		...
		it('should return an empty array when the response is not ok', async () => {
			...
			expect(console.error).toHaveBeenCalledWith('Wrong response from getDrivers');
			expect(result).toEqual([]);
		});
